...
def add_customer(request):
    cart = Cart(request)
    print('my cart', cart)
    print('my cart total',cart.get_total_price())
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            customer = form.save()
        return render(request, 'orders/payment.html', {'email':customer.email, 'id':customer.id, 'cart':cart})
...
def save_item(request, id):
    ...
    for item in cart:
        OrderItem.objects.create(customer=customer, product=item['product'], price=item['price'], quantity=item['quantity'])
    # clear the cart
    cart.clear()
...
def processpayment(request):
    ...
    if request.method == 'POST':
        ...
        success_url = request.build_absolute_uri(reverse('orders:save_item', kwargs={'id': customerid}))
        ...
        metadata= json.dumps({"payment_id":customerid, "cancel_action":cancel_url, })
       
        # Paystack checkout session data
        api_key= settings.PAYSTACK_TEST_SECRET_KEY
        
        session_data = {'email': email,'amount': int(float(amount)), 'callback_url': success_url, 'metadata': metadata }
